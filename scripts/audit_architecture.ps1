...
$repeatQueuePath = Join-Path $RepoRoot "scripts\win32_lectern0_page_repeat_queue_smoke.ps1"
$pageTurnPath = Join-Path $RepoRoot "scripts\win32_lectern0_page_turn_regression_smoke.ps1"
...
if ($failures.Count -eq 0) {
  ...
  if ($app.IndexOf('Lectern0PageRepeatFrameRate = 60') -lt 0 -or
      ...
      $app.IndexOf('Lectern0PageRepeatTimerId') -ge 0) {
    $failures.Add("lectern0 held navigation must retain bounded wall-clock 60 Hz 24/3 policy, hold-scoped timer resolution with transition and exit cleanup, no presentation-frame clock, and no WM_TIMER repeat driver")
  }
  ...
  if ($repeatQueue.IndexOf('--page-repeat-win32-smoke') -lt 0 -or
    ...
    $failures.Add("Lectern0 must lock the real Win32 60 Hz 24/3 queue cadence twice")
  }
  if ($app.IndexOf('Lectern0PageRepeatProbeMoveCount = 12') -lt 0 -or
      $app.IndexOf('Lectern0GotmMinimumProseSpineBytes = 128') -lt 0 -or
      $app.IndexOf('Lectern0GotmMinimumProseTextBytes = 8') -lt 0 -or
      $app.IndexOf('Lectern0GotmMinimumProseTextRows = 1') -lt 0 -or
      $app.IndexOf('Lectern0PresentationRetryTimerId = 3') -lt 0 -or
      $app.IndexOf('lectern0_page_action_schedule_presentation_retry') -lt 0 -or
      $app.IndexOf('page_action_waiting_for_present') -lt 0 -or
      $app.IndexOf('page_action_pending') -lt 0 -or
      $app.IndexOf('Lectern0PageRepeatWin32Probe_ShiftModifier') -lt 0 -or
      $app.IndexOf('lectern0_gotm_navigation_frame_is_canonical_nonempty') -lt 0 -or
      $app.IndexOf('lectern0_gotm_page_start_is_word_boundary') -lt 0 -or
      $app.IndexOf('Lectern0PageRepeatQueueDrainCap = 32') -lt 0 -or
      $app.IndexOf('Lectern0PageRepeatProbeQueueDrainCap = Lectern0PageRepeatQueueDrainCap') -lt 0 -or
      $app.IndexOf('drained_message_count >= Lectern0PageRepeatQueueDrainCap') -lt 0 -or
      $app.IndexOf('state_save_transaction_success_count') -lt 0 -or
      $app.IndexOf('lectern0_page_repeat_probe_file_snapshot') -lt 0 -or
      $app.IndexOf('lectern0_page_repeat_probe_paint_proc') -lt 0) {
    $failures.Add("Lectern0 page-repeat proof must cover sustained moves, bounded queue drain, a paired host save with individually atomic state/catalog files, and real paint dispatch")
  }
  if ($pageTurn.IndexOf('invalid_word_start_pages=0/0') -lt 0 -or
    ...
    $failures.Add("Lectern0 exact-book page-turn proof must lock non-orphan prose, raw UTF-8 word starts, and deferred reversal key-up semantics")
  }
