...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "Octavo Find contrast smoke failed`n$Tail"
}
$PassLine = Get-Content -LiteralPath $LogPath | Where-Object {
  $_ -match '^octavo_reader_view_find_active_contrast result=pass '
} | Select-Object -Last 1
if (!$PassLine -or $PassLine -notmatch 'checkpoint=5 ' -or
    $PassLine -notmatch 'query=Paran active_index=2 themes=6 ') {
  throw "Octavo Find contrast result is incomplete: $PassLine"
}
...
$Lines = Get-Content -LiteralPath $LogPath | Where-Object {
  $_ -match '^octavo_reader_view_find_active_contrast theme='
}
...
foreach ($Line in $Lines) {
  $Match = [regex]::Match(
    $Line,
    '^octavo_reader_view_find_active_contrast theme=([^ ]+) active=([0-9A-F]{6}) inactive=([0-9A-F]{6}) active_ranges=(\d+) inactive_ranges=(\d+) active_draws=(\d+) inactive_draws=(\d+) active_pixels=(\d+) inactive_pixels=(\d+) bmp=(.+)$')
...
Write-Host $PassLine
Write-Host "win32_octavo_find_active_contrast_smoke result=pass themes=6 summary=$SummaryPath"
