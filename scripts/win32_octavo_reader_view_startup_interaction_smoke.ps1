...
if ($LASTEXITCODE -ne 0) {
  $output | Write-Host
  throw "octavo startup interaction smoke failed with exit code $LASTEXITCODE"
}
$line = $output | Where-Object {
  $_ -match '^octavo_reader_view_startup_interaction result=pass '
...
if (!$line -or
  ...
  $output | Write-Host
  throw "octavo startup interaction smoke did not report the required contract"
}

Write-Host $line
Write-Host "win32_octavo_reader_view_startup_interaction_smoke result=pass"
