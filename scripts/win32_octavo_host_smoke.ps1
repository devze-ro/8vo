...
$Epub = Join-Path $Out "octavo_slice1.epub"
...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="bookid">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    <dc:identifier id="bookid">octavo-slice1</dc:identifier>
    <dc:title>Octavo Slice 1</dc:title>
...
<ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1">
  <head><meta name="dtb:uid" content="octavo-slice1"/></head>
  <docTitle><text>Octavo Slice 4B</text></docTitle>
  <navMap>
...
if ($LASTEXITCODE -ne 0) {
  $output | Write-Host
  throw "octavo headless host smoke failed with exit code $LASTEXITCODE"
}
$line = ($output | Where-Object { $_ -match '^octavo_host_smoke result=pass ' } | Select-Object -Last 1)
if (!$line) {
  $output | Write-Host
  throw "octavo headless host smoke did not report pass"
}
if ($line -notmatch 'carets=frozen18x32') {
  $output | Write-Host
  throw "octavo headless host smoke did not lock the frozen page-caret bridge"
}
...
if ($missingExit -eq 0 -or
    !($missingOutput | Where-Object { $_ -match '^octavo_host_smoke result=fail reason=open$' })) {
  $missingOutput | Write-Host
  throw "octavo invalid-path host smoke did not fail visibly"
}

Write-Host $line
Write-Host "win32_octavo_host_smoke result=pass fixture=$Epub"
