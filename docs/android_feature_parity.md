...
Last reviewed: 2026-08-07. Baseline implementation: accepted Android Ports 0-7
...
| Multi-color highlights | Improve | Port 11 defines the portable same-spine Reader0 byte-range record, four-value semantic palette, causal mutation, tombstone, and bounded deterministic merge contract; Android creation/rendering remains to be implemented after the local bookmark slice. | Theme-tuned accessible palette, overlap policy, exact anchor durability, and export semantics. |
| Notes | Match | Port 11 defines bounded note records and a causal multi-value merge that retains every concurrent body; editing and conflict-resolution UI remain to be implemented. | Create/edit/delete, visible association, autosave/recovery, and conflict-safe durable storage. |
...
| Annotations workspace | Match | Port 11 begins with a bounded current-book Bookmarks sheet and stable Go to/Remove actions. Highlight/note filters, conflict resolution, search, and export remain missing. | Book/chapter grouping, snippets, color/type/date filters, favorites, notes, conflicts, and direct navigation. |
...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| TalkBack | Improve | Port 8 retains the bounded reader virtual tree and native Contents hierarchy. Port 9 adds labelled Find controls, results, current-result state, polite live status, deterministic focus return, and 48dp actions. API 36 and API 34 semantics/focus automation pass, including the iQOO 27/27 large-text/reduced-motion search/navigation matrix. With TalkBack 17.0.1, the physical user completed audible search-sheet exploration and Port 10 Select text, Copy selected text, and Clear selection actions without reporting an issue. The cross-page follow-up adds Extend selection to previous/next page with success and chapter-boundary announcements; its API 36 automation passes, and the physical user confirmed Select, both extension directions, and Copy were all available without reporting an issue. Accessibility, rotation, audio, font, animation, and 8vo data state restored to the captured baseline. | Extend publication semantics with accurate headings, lists, links, alt text, tables, notes, and language; complete broader whole-app audible review. |
