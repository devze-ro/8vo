...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Text selection | Match | Port 10 adds Reader0-authoritative session-local single-page word selection, rendered-row/style/justification hit geometry, compact handles with 48dp touch regions, Android contextual Copy, clipboard-failure preservation, selection-first system Back, and virtual-page Select/Copy/Clear actions. API 36 passes 5/5 focused, 95/95 ordinary, 32/32 at 130% text/reduced motion, restart, empty-crash, and Paper/Warm dark/High Contrast visual gates. API 34 passes 5/5 focused, 95/95 ordinary, restart, and the correctly configured 32/32 system-130%-text/reduced-motion matrix. Physical same-page touch/Copy/Back and audible TalkBack actions pass. Activity/process replacement and navigation clear selection; no Java text anchors or EPUB interpretation are introduced. Physical review confirmed selection cannot continue across pages. | Treat cross-page auto-turn/continuation as a launch blocker and define same-spine versus cross-spine ownership before bookmarks/highlights/notes depend on the anchor contract; then add paragraph expansion, fuller Unicode segmentation, and durable anchors. |
...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| TalkBack | Improve | Port 8 retains the bounded reader virtual tree and native Contents hierarchy. Port 9 adds labelled Find controls, results, current-result state, polite live status, deterministic focus return, and 48dp actions. API 36 and API 34 semantics/focus automation pass, including the iQOO 27/27 large-text/reduced-motion search/navigation matrix. With TalkBack 17.0.1, the physical user completed audible search-sheet exploration and Port 10 Select text, Copy selected text, and Clear selection actions without reporting an issue. Accessibility, permission, rotation, audio, font, animation, and 8vo data state restored to the captured baseline. | Extend publication semantics with accurate headings, lists, links, alt text, tables, notes, and language; complete broader whole-app audible review. |
| Large text/display scale | Improve | Port 7 offers six reader sizes from 14sp through 28sp. Port 9's selected search/navigation/accessibility matrix passes 27/27 at 130% system text on API 36 and API 34. Port 10's final correctly configured system-font run passes 32/32 at 130% on API 34 in 34.958 seconds; an earlier unused global-font-key attempt is explicitly excluded. The final baseline has system font `1.0` and no global font key. Broader-device validation remains pending. | No clipped/hidden actions at supported scales; broaden device coverage. |
...
| Reduced motion | Improve | Port 9's search/navigation matrix passes 27/27 with system animations disabled on API 36 and API 34. Port 10's final corrected API 34 matrix passes 32/32 in 34.958 seconds. Stored scales were `1.0` before the run while vivo Recents motion was absent; republishing did not help, but restarting Launcher did. Cleanup therefore restored every scale explicitly to `1.0`, restarted Launcher, and verified 47 Launcher plus 74 SystemUI frames. The crash buffer remained empty. | Coordinate physical runs with the user; do not disable device-wide animations outside the test window. Restore window, transition, and animator scales explicitly to `1.0`, restart the affected vivo Launcher when required, and verify visible behavior as well as stored values. Honor system preference and provide a non-animated equivalent for every transition. |
