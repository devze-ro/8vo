...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Offline-first operation | Improve | Port 11 keeps every mutation local-first. Its qualified annotation family has actor-neutral `O1AP` plus disconnected caller-driven `O1AS` coordination. The separate qualified position family has bounded canonical `O1RP` per-device lanes, private atomic `O1RS` decisions, clock-free merge/stale replay, exact successful-presentation publication, and visible retry. The seventh appearance-only [`O1PF`/`O1SS` contract](android_port11_preference_sync.md) is accepted on API 36. The eighth independent [`O1PC`/`O1PS` progress-choice contract](android_port11_progress_sync.md) is also accepted on API 36 using deterministic simulated remote bytes while preserving the separate local `O8PG` file: focused passes 37/37 in 78.279 seconds, coexistence 49/49 in 143.490 seconds, ordinary 238/238 in 438.899 seconds, and external force-stop recovery with empty crash/fatal buffers. No provider, account, permission, worker, thread, clock, physical device, or transport is connected. | Extend the same provider-neutral discipline to catalog and transfer queues while every core feature remains understandable offline. |
