...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| TalkBack | Improve | Port 8 retains the bounded reader virtual tree and native Contents hierarchy. Port 9 adds labelled Find controls, results, current-result state, polite live status, deterministic focus return, and 48dp actions. API 36 and API 34 semantics/focus automation pass, including the iQOO 27/27 large-text/reduced-motion search/navigation matrix. With TalkBack 17.0.1, the physical user completed audible search-sheet touch exploration, result activation/Next, Back, announcements, and focus return without reporting an issue. All accessibility, permission, rotation, audio, font, animation, and 8vo data state was then restored to its captured baseline. | Extend publication semantics with accurate headings, lists, links, alt text, tables, notes, and language; complete broader whole-app audible review. |
| Large text/display scale | Improve | Port 7 offers six reader sizes from 14sp through 28sp. Port 9's final selected search/navigation/accessibility matrix passes 27/27 at 130% system text on API 36 in 30.401 seconds and on the API 34 iQOO in 27.219 seconds of instrumentation time. The current iQOO run began and ended with font, window, transition, and animator scales explicitly at `1.0`. A prior Port 8 database restoration to an absent animator key was byte-exact but did not restore all vivo SystemUI behavior; Port 9 therefore restored animator duration scale explicitly and verified live behavior. Broader-device validation remains pending. | No clipped/hidden actions at supported scales; broaden device coverage. |
...
| Reduced motion | Improve | Port 9's search/navigation matrix passes 27/27 with system animations disabled on API 36 and API 34. The coordinated iQOO run took 27.219 seconds of instrumentation time, restored font and all three animation scales explicitly to their original `1.0` values, and then exercised vivo SystemUI through 74 rendered notification-shade frames to verify animations were behaviorally active. The crash buffer remained empty. User subjective quality and automatic system-preference following remain pending. | Coordinate physical runs with the user; do not disable device-wide animations outside the test window. Restore window, transition, and animator scales explicitly to `1.0`, then verify visible behavior as well as stored values. Honor system preference and provide a non-animated equivalent for every transition. |
