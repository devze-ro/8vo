...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Offline-first operation | Improve | Port 11 makes the first annotation slice entirely local and requires its portable merge gates before any provider connection. | Every core feature queues safe local changes and remains understandable without connectivity. |
...
| Position/settings/annotations sync | Improve | Local per-book presented positions plus one checksummed version-3 global appearance record. Port 11 adds a provider-neutral, digest-keyed annotation envelope with stable record/mutation/tombstone identities and device-clock-independent causal merge; provider synchronization remains missing. | Pass deterministic offline record/merge gates, then add explicit Google Drive transport, conflict UI, and migration. |
