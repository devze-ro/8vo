...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Offline-first operation | Improve | Port 11 keeps every mutation local-first. Its qualified annotation family has actor-neutral `O1AP` plus disconnected caller-driven `O1AS` coordination. The separate qualified position family has bounded canonical `O1RP` per-device lanes, private atomic `O1RS` decisions, clock-free merge/stale replay, exact successful-presentation publication, and visible retry. On both API 36 and API 34, position coverage passes 24/24 focused, external force-stop recovery, and 158/158 ordinary tests. No provider, account, permission, worker, thread, clock, or transport is connected. | Extend the same provider-neutral discipline to catalog, preferences, and transfer queues while every core feature remains understandable offline. |
