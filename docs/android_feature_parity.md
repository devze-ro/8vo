...
Last reviewed: 2026-08-08. Baseline implementation: accepted Android Ports 0-7
...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Offline-first operation | Improve | Port 11 keeps every mutation local-first and now has an API 36-qualified actor-neutral `O1AP` codec plus a disconnected caller-driven coordinator. The private atomic `O1AS` retry/review state stages bounded untrusted bytes, gates first merge and remote recreation, retains join-limit inputs, and drives conditional create/replace without clocks, threads, or a provider interface. The coordinator passes 7/7, including the exact 16 MiB-minus-44 snapshot on the 192 MiB heap; all 134 ordinary tests pass. No provider, account, permission, worker, or UI is connected. | Extend the same provider-neutral discipline to catalog, preferences, positions, and transfer queues while every core feature remains understandable offline. |
...
| Position/settings/annotations sync | Improve | Local per-book presented positions plus one checksummed version-3 global appearance record. Annotation sync alone now has a qualified provider-neutral boundary: stable record/mutation/tombstone identities, frozen `O1AP` bytes/goldens, device-clock-independent join, conditional-revision command semantics, durable pending/error/review state, and exact restart recovery. No transport executes those commands. Position prompting and merge, preferences, and Library identity remain separate later record families. | Add each remaining portable family with its own bounds/merge policy, then explicit Google Drive transport/migration, including a Kindle-like choice before applying a newer remote presented-position anchor. |
