...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Offline-first operation | Improve | Port 11 keeps annotations entirely local and now has an API 36-qualified actor-neutral `O1AP` codec, bounded untrusted decoder/serializer, atomic import rollback, and deterministic opposite-frontier join. No provider is connected. | Extend the same provider-neutral discipline to catalog, preferences, positions, and transfer queues while every core feature remains understandable offline. |
...
| Position/settings/annotations sync | Improve | Local per-book presented positions plus one checksummed version-3 global appearance record. Port 11 adds an offline-qualified, provider-neutral, digest-keyed annotation envelope with stable record/mutation/tombstone identities, frozen canonical bytes/golden data, and a device-clock-independent causal join; provider synchronization remains missing. Position prompting and merge remain a separate later record family. | Add the remaining portable families and explicit Google Drive transport/migration, including a Kindle-like choice before applying a newer remote presented-position anchor. |
