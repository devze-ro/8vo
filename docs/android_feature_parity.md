...
portrait Navigation visual review passes. Whole-app audible TalkBack, full-app visual
parity, remaining bounded polish, and user subjective/manual acceptance remain
pending. Port 9 bounded search now also passes API 36 and API 34 automation,
confirmed-force-stop recovery, physical reduced-motion/large-text checks, and
a representative real-book search/jump/highlight review.
Port 11's provider-neutral catalog/managed-transfer slice is accepted on API 36;
it does not yet connect a provider or close physical, account, network,
permission, or security gates.
...
| --- | --- | --- | --- |
| Local EPUB import | Improve | Android document picker; fixed staging; incremental SHA-256; Reader0 validation; journaled same-directory atomic managed publication; exact Port 6 association | Multi-select and visible import progress; preserve the qualified duplicate, invalid, full-storage, and retry outcomes. |
| Multiple-book catalog | Match | Built-in sample plus 63 imported books; separate provider-neutral add-only `O1LC` discovery with private `O1LS` review state | Scale without losing bounded/recoverable storage guarantees; migrate catalog versions safely; connect only a separately authorized provider. |
| Exact per-book resume | Improve | Port 6 accepts exact same-layout resume. The accepted Port 7 emulator and iQOO 36/36 matrices plus separate ProcessRestart driver passed around the last successfully presented Reader0 spine/byte anchor. Port 11 now independently keys bounded `O1RP`/`O1RS` state by exact EPUB SHA-256, strictly rejects a clamped or over-bound Reader0 restore, and records or accepts a synchronized anchor only after its exact semantic anchor is posted successfully. The final API 36 and API 34 position classes both pass 24/24 plus confirmed external force-stop recovery; the physical gate also closed touch-mode action focus. | Preserve the qualified semantic location across application-version and representative-book changes, complete hands-on confirmation accessibility review, then connect only an explicitly authorized provider adapter. |
...
| Remove | Improve | **Remove from this device** uses durable `O1BQ` cleanup, removes the Port 6 row and exact managed copy, retains the provider source, and records private `O1LS LOCAL_REMOVED`; a suppression failure remains visible without rolling back local removal | Add confirmation or undo and define a separately reviewed synchronized withdrawal/cloud-deletion contract; never infer it from local absence. |
| Locate/re-import/repair | Improve | Port 11 detects missing, unequal, and repair-required managed bytes and offers explicit origin-bound local repair through deterministic catalog/manifest inputs without overwriting mismatching bytes | Connect a separately authorized provider, preserve the accepted cleanup/identity gates, and broaden user-facing locate/re-import recovery. |
...
| Font size | Match | Port 8 defaults new, missing, or corrupt global state to 16sp and offers 14, 16, 18, 21, 24, and 28sp. Valid version-1 18sp and version-2 16/18sp default-like records yield 16sp in memory, preserve every other field, mark migration pending, and leave old bytes exact. Every valid version-3 choice, including 14sp, remains exact because the record has no default-versus-explicit provenance. Version 3 publishes only after the first accepted reader frame; atomic-save failure remains pending, visible, and retryable. The corrected legacy API 36 appearance regression passes 36/36 in 126.493 seconds after its corrected pair passed 2/2 in 9.457 seconds; earlier API 36/API 34 migration, persistence, fresh-default, and 130%-system-text automation also passes. Hands-on scale review remains pending. | Immediate preview, broader accessible range where justified, broader physical scale/device evidence, and exact semantic resume. |
...
| Page and progress display | Match | Port 8 offers Chapter, meaningful Page, canonical Location, and Percentage choices from the last successfully presented Reader0 state. The global choice and current position publish only after accepted presentation. Corrected API 36 and API 34 physical reflow, lifecycle, surface-replacement, restart, and reopen gates pass; controlled real-book Chapter navigation and exact Return also passed. Port 11's eighth independent local slice now accepts provider-neutral choice synchronization on API 36: bounded canonical `O1PC` lanes, private atomic `O1PS` review/pending state, and the local `O8PG` record remain separate; focused tests pass 37/37, coexistence passes 49/49, ordinary tests pass 238/238, and external force-stop recovery passes with empty crash/fatal buffers. No physical device or provider was used for that sync slice. | Complete audible accessibility and broader representative-book review; retain the local acceptance gates before any authorized transport is connected. |
...
| In-book full-text search | Match | Port 9 adds bounded session-local Reader0 search with contextual snippets and section labels, explicit retained/total and truncation state, direct/previous/next navigation, presentation-gated Search history, and distinct active/inactive page emphasis. Query, clear, navigation failure, Activity recreation, and confirmed process-restart behavior pass on API 36 and API 34; Android retains at most 64 results and does not persist the query. On the iQOO, focused search passed 5/5, the ordinary matrix 90/90, and the selected accessibility matrix 27/27. A representative *Gardens of the Moon* query for `Paran` reported 630 matches, disclosed the first 64, moved from 0% to 3% on a Chapter One result, advanced with Next, and kept the active hit distinct on-page. Physical TalkBack/touch exploration also passes for the Find controls, status, current/ordinary results, activation, Next, Back, open/close announcements, and focus return. | Add Unicode-aware case folding/tokenization and a durable local index without blocking ordinary reading; complete broader-book review. |
...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Text selection | Match | Port 10 adds Reader0-authoritative session-local word selection, rendered-row/style/justification hit geometry, compact handles with 48dp touch regions, Android contextual Copy, clipboard-failure preservation, selection-first system Back, and virtual-page actions. The follow-up uses Reader0's public preserved-selection page move to continue either handle across successfully presented pages in the same spine, repeat after edge dwell, expose independent endpoint visibility, and stop explicitly at a chapter boundary; it does not add Java anchors or EPUB interpretation. A native loupe tracks successfully presented drag frames without flicker, keeps the active row centered, and dismisses at gesture/lifecycle boundaries. API 36 passes 10/10 focused selection in 73.180 seconds, 100/100 ordinary in 490.402 seconds, restart, 37/37 at 130% text/reduced motion in 101.257 seconds, empty-crash, and Paper/Warm dark/High Contrast visual gates. API 34 passes 10/10 focused in 16.799 seconds, 100/100 ordinary in 178.812 seconds, restart, 37/37 at 130% text/normal motion in 39.059 seconds, touch/loupe review, and bounded audible TalkBack extension actions. | Keep cross-spine selection explicit until Reader0 owns such an anchor; then add paragraph expansion, fuller Unicode segmentation, and durable anchors. |
| Multi-color highlights | Improve | Port 11 implements the local Android slice on the portable same-spine Reader0 byte-range record: Yellow/Pink/Blue/Orange selection actions, theme-tuned native rendering, deterministic record-ID overlap precedence, atomic create/recolor/remove, restart durability, TalkBack actions, and presentation-gated selection clearing. API 36 passes 10/10 highlight/store focus, 24/24 combined regression, 113/113 ordinary in 193.044 seconds, exact Orange-highlight fresh-process recovery, 29/29 at 130% text/reduced motion, and 5/5 fixed-pagination reduced motion. API 34 passes 10/10 focus, a clean 113/113 ordinary rerun in 234.374 seconds, exact external restart, 29/29 at 130% text and 5/5 at 100% text with normal motion, touch review, bounded TalkBack 17.0.1 review, empty final crash state, live-motion verification, and exact restoration. Provider synchronization and export remain disconnected. | Retain exact anchors/colors through deterministic synchronization; add publisher-aware export semantics. |
| Notes | Match | Port 11's local note candidate adds selection-to-note, a UI0-inset 4,096-byte native editor, exact create/edit/delete/cancel, an independently atomic crash-recovery draft, failure rollback/retry, ordered workspace rows/navigation, explicit multi-body conflict resolution, and a small theme-safe reader marker at the exact Reader0 range end. A presented marker has a 48dp-class generation-matched target that opens its exact durable note without replacing another draft. The revised API 36 candidate passes 13/13 focused tests with real marker-tap/pixel/lifecycle/failure evidence and all 119 ordinary tests; API 34 passes the same 13/13 focus and a clean 119/119 ordinary rerun with normal motion. Exact restart, crash/settings restoration, byte-exact device cleanup, focused touch acceptance, and the user's bounded TalkBack note-workflow review pass. Google Drive remains disconnected. | Retain exact bodies/conflicts through deterministic synchronization; add broader annotation filters/export. |
| Bookmarks | Match | Port 11 implements the local-only slice: digest-keyed Reader0 point anchors, atomic toggle/removal, restart durability, ordered list, and presentation-gated direct navigation. API 36 and API 34 focused, ordinary, external-restart, 130%-text, and empty-crash gates pass. Physical touch and TalkBack 17.0.1 bookmark/workspace review pass without a reported issue. Google Drive remains disconnected. | Retain the same record through deterministic synchronization after the provider-neutral offline gates remain green. |
| Annotations workspace | Match | Port 11's bounded current-book Annotations sheet combines bookmark, highlight, and dual-target-qualified note counts/rows; headings; excerpts; named swatches; UI0-inset note editing; note bodies/conflicts; Go to/Edit/recolor/Remove; and Done. Reader pages show the physically accepted product-owned exact-anchor note glyph, and tapping it opens the exact note editor. Broader filters, search, and export remain missing. | Book/chapter grouping, type/color/date filters, favorites, local query, and export. |
...
| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| TalkBack | Improve | Port 8 retains the bounded reader virtual tree and native Contents hierarchy. Port 9 adds labelled Find controls, results, current-result state, polite live status, deterministic focus return, and 48dp actions. API 36 and API 34 semantics/focus automation pass, including the iQOO 27/27 large-text/reduced-motion search/navigation matrix. With TalkBack 17.0.1, the physical user completed audible search-sheet exploration and Port 10 selection/cross-page actions without reporting an issue. Port 11 physical review accepts all four named highlight actions and success state plus the Annotations heading/count/color/excerpt, recolor controls, Go to, Remove, and Done. Accessibility, rotation, audio, font, animation, APKs, and 8vo data state restored to the captured baseline. | Extend publication semantics with accurate headings, lists, links, alt text, tables, notes, and language; complete broader whole-app audible review. |
| Large text/display scale | Improve | Port 7 offers six reader sizes from 14sp through 28sp. Port 9's selected search/navigation/accessibility matrix passes 27/27 at 130% system text on API 36 and API 34. Port 10's final correctly configured system-font run passes 32/32 at 130% on API 34 in 34.958 seconds; the cross-page follow-up passes 37/37 on API 36 in 101.257 seconds and API 34 in 39.059 seconds. An earlier unused physical global-font-key attempt remains excluded. Both targets restored to system font `1.0`. | No clipped/hidden actions at supported scales; broaden device coverage. |
...
| Reduced motion | Improve | Port 9's search/navigation matrix passes 27/27 with system animations disabled on API 36 and API 34. Port 10's final corrected API 34 baseline matrix passes 32/32 in 34.958 seconds, and the cross-page API 36 matrix passes 37/37. Stored physical scales were `1.0` while vivo Recents motion was absent; republishing did not help, but restarting Launcher did. Cleanup therefore restored every physical scale explicitly to `1.0`, restarted Launcher, and verified 47 Launcher plus 74 SystemUI frames. The cross-page and Port 11 physical follow-ups deliberately retained normal motion at `1.0`; Port 11 cleanup recorded 123 Launcher and 107 SystemUI frames. | Coordinate physical runs with the user; do not disable device-wide animations outside the test window. Restore window, transition, and animator scales explicitly to `1.0`, restart the affected vivo Launcher when required, and verify visible behavior as well as stored values. Honor system preference and provide a non-animated equivalent for every transition. |
...
The ninth independent Port 11 local slice accepts provider-neutral catalog
discovery and managed EPUB transfer on API 36. Android compilation is green;
...
63.351 seconds and font plus all animation scales are restored to `1.0`. The
final crash/fatal buffers are empty. No physical device, provider, Drive/account
path, network permission, security decision, worker, cloud resource, or Play
Console operation was part of the gate.

| Capability | Target | Current Android baseline | Completion requirement |
| --- | --- | --- | --- |
| Offline-first operation | Improve | Port 11 keeps every mutation local-first. Annotation `O1AP`/`O1AS`, position `O1RP`/`O1RS`, appearance `O1PF`/`O1SS`, progress choice `O1PC`/`O1PS`, local `O8PG`, and catalog/transfer `O1LC`/`O1LS`/`O1BM`/`O1BQ` remain separate. The ninth slice leaves reading/import successful when discovery publication fails and leaves exact durable transfer/cleanup attention when later reconciliation fails. Offer Back durably records **Not now**; retained work/retry Back only defers the modal and exposes a nonmodal exact reopen action. | Preserve these independent offline contracts while adding an explicitly authorized provider; do not let authorization or network state block reading or local mutations. |
...
| Books sync | Improve | The accepted local-only [`O1LC`/`O1LS`/`O1BM`/`O1BQ` contract](android_port11_catalog_transfer.md) provides digest identity, reviewed discovery, sequential resumable simulated transfer, atomic managed publication, durable cleanup/repair, and explicit retained retry. Late publication recovery runs Reader0 first, then fused attempt-bound 4 MiB SHA-256 and Port 6 catalog commit steps. It performs no upload or provider operation. | Add explicit provider authorization, remote ownership/readback proof, metered/quota policy, account switching, cloud deletion, and physical-device qualification without weakening local recovery or creating duplicate copies. |
